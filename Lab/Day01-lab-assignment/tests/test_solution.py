...
# Valid Python module name (DAY_DIR.name may contain hyphens, breaking mock.patch)
_MODULE_PREFIX = "day01_lab"

if (SOLUTION_DIR / "solution.py").exists():
    _m = _load(SOLUTION_DIR / "solution.py", f"{_MODULE_PREFIX}.solution")
elif (SOLUTION_DIR / "app.py").exists():
    _m = _load(SOLUTION_DIR / "app.py", f"{_MODULE_PREFIX}.solution")
else:
    src = "template.py" if (DAY_DIR / "template.py").exists() else "app.py"
    _m = _load(DAY_DIR / src, f"{_MODULE_PREFIX}.template")
