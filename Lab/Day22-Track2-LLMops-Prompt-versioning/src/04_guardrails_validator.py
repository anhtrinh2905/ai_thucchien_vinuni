...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        if found_pii:
            ...
            # OnFailAction.FIX chỉ thay thế output qua fix_value của FailResult;
            # PassResult.value_override KHÔNG được áp dụng ở guardrails 0.10.x.
            return FailResult(
                error_message=f"Phát hiện {len(found_pii)} PII: {[p[0] for p in found_pii]}",
                fix_value=redacted_text,
            )

        # Không có PII → output giữ nguyên
        return PassResult()
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        # Thử sửa JSON rồi parse lại — dùng FailResult.fix_value để OnFailAction.FIX
        # thực sự thay thế output (PassResult.value_override không áp dụng ở guardrails 0.10.x).
        try:
            repaired_text = self._repair(value)
            parsed        = json.loads(repaired_text)
            print(f"  🔧 JSON đã được sửa thành công")
            return FailResult(
                error_message="JSON lỗi — đã tự động sửa",
